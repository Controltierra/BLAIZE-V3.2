  ...
  if (onoff) {
    ...
    case 15: 
      ...
      if (mediaPipeDataReceived) {
        ...
        // Mostrar nombre del logo actual
        fill(200, 200, 200);
        textSize(16);
        ...
        // Info si tracker no conectado
        if (udpPacketsReceived > 0) {
          fill(255, 150, 0);
          textSize(16);
          text("Recibiendo datos pero sin pose detectada", frameSizeX/2, 70);
          text("Ponte frente a la cámara", frameSizeX/2, 90);
        }
      }
      break;
...
void drawLoadLogosButton() {
  int btnX = frameSizeX + 10;
  int btnY = 600;
  int btnW = 450;
  int btnH = 50;
  ...
  // Dibujar botón
  fill(mouseOver ? color(80, 180, 255) : color(50, 150, 220));
  stroke(255);
  strokeWeight(2);
  ...
  textAlign(CENTER, CENTER);
  textSize(18);
  text("CARGAR IMÁGENES A GALERÍA", btnX + btnW/2, btnY + btnH/2 - 8);
  textSize(12);
  text("Logos actuales: " + logoGallery.length, btnX + btnW/2, btnY + btnH/2 + 12);
  
  // Detectar clic
  if (mouseOver && mousePressed && flag) {
    flag = false;
    selectInput("Selecciona imágenes para la galería:", "filesSelected");
  }
}
  ...
  if (selection == null) {
    ...
    try {
      ...
      if (ext.equals("png") || ext.equals("jpg") || ext.equals("jpeg") || ext.equals("gif")) {
        // Copiar archivo a data/logos/
        File destFolder = new File(dataPath("logos"));
        if (!destFolder.exists()) {
          destFolder.mkdirs();
        }
        
        File destFile = new File(dataPath("logos/" + fileName));
        
        // Si el archivo ya existe, añadir número
        int counter = 1;
        while (destFile.exists()) {
          String nameWithoutExt = fileName.substring(0, fileName.lastIndexOf('.'));
          destFile = new File(dataPath("logos/" + nameWithoutExt + "_" + counter + "." + ext));
          counter++;
        }
        
        // Copiar archivo
        byte[] fileData = loadBytes(sourcePath);
        saveBytes(destFile.getAbsolutePath(), fileData);
        
        println("✓ Imagen copiada: " + destFile.getName());
        
        // Recargar galería
        loadLogoGallery();
        println("✓ Galería recargada. Total de logos: " + logoGallery.length);
        
      } else {
        println("✗ Formato no válido. Usa PNG, JPG, JPEG o GIF");
      }
    } catch (Exception e) {
      println("✗ Error al cargar imagen:");
      e.printStackTrace();
    }
  }
}
